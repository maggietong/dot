#Embedded file name: _ipyutils.py
...
from IPython.utils.traitlets import HasTraits, Integer, Instance, Unicode, Dict, List, Bool, Set, Any
...
class ExecuteReply(RichOutput):
    ...
    def _repr_pretty_(self, p, cycle):
        ...
        if '\n' in text_out and not text_out.startswith('\n'):
            text_out = '\n' + text_out
        p.text(out + u'Out[%i:%i]: ' % (self.metadata['engine_id'], self.execution_count) + normal + text_out)
...
class Client(HasTraits):
    ...
    def __init__(self, url_file = None, profile = None, profile_dir = None, ipython_dir = None, context = None, debug = False, sshserver = None, sshkey = None, password = None, paramiko = None, timeout = 10, cluster_id = None, **extra_args):
        if profile:
        ...
        if url_file is None:
            raise ValueError("I can't find enough information to connect to a hub! Please specify at least one of url_file or profile.")
        ...
        if location is not None and addr == localhost():
            if not is_local_ip(location) and not sshserver:
                sshserver = cfg['ssh']
            if not is_local_ip(location) and not sshserver:
                warnings.warn("\n            Controller appears to be listening on localhost, but not on this machine.\n            If this is true, you should specify Client(...,sshserver='you@%s')\n            or instruct your controller to listen on an external IP." % location, RuntimeWarning)
        elif not sshserver:
            sshserver = cfg['ssh']
        self._config = cfg
        ...
        try:
            ...
            msg = '\n'.join(["Connection file is invalid (missing '{}'), possibly from an old version of IPython.", 'If you are reusing connection files, remove them and start ipcontroller again.'])
        ...
        if actual_ipython and 'px' not in ip.magics_manager.magics:
            self.activate()
    ...
    def _update_engines(self, engines):
        ...
        if sorted(self._engines.keys()) != list(range(len(self._engines))) and self._task_scheme == 'pure' and self._task_socket:
    ...
    def shutdown(self, targets = 'all', restart = False, hub = False, block = None):
        ...
        for t in targets:
            self.session.send(self._control_socket, 'shutdown_request', content={'restart': restart}, ident=t)
    ...
    def send_apply_request(self, socket, f, args = None, kwargs = None, metadata = None, track = False, ident = None):
        ...
        bufs = serialize.pack_apply_message(f, args, kwargs, buffer_threshold=self.session.buffer_threshold, item_threshold=self.session.item_threshold)
        msg = self.session.send(socket, 'apply_request', buffers=bufs, ident=ident, metadata=metadata, track=track)
    ...
    def send_execute_request(self, socket, code, silent = True, metadata = None, ident = None):
        ...
        msg = self.session.send(socket, 'execute_request', content=content, ident=ident, metadata=metadata)
        msg_id = msg['header']['msg_id']
        self.outstanding.add(msg_id)
        if ident:
            if isinstance(ident, list):
    ...
    def result_status(self, msg_ids, status_only = True):
        ...
        for msg_id in sorted(theids):
            if msg_id in content['completed']:
                ...
                md = self.metadata[msg_id]
                md_msg = dict(content=rcontent, parent_header=parent, header=header, metadata=rec['result_metadata'])
